...
<div class="flex flex-col gap-6" x-data="employeeManagement()">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-text-light dark:text-text-dark text-3xl font-black tracking-tight">Employees</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage staff, salaries, and advances.</p>
        </div>
        ...
        @forelse($employees as $employee)
            <div class="bg-container-light dark:bg-container-dark rounded-xl shadow-subtle p-6 flex flex-col gap-4 relative group hover:shadow-lg transition-all">
                ...
                <div class="absolute top-4 right-4" x-data="{ open: false }">
                    ...
                    <div x-show="open" class="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-md shadow-xl py-1 z-10 border border-gray-100 dark:border-gray-700">
                        <button @click="openEditModal({{ $employee }})" class="flex w-full items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">
                            <span class="material-symbols-outlined text-lg mr-2">edit</span> Edit
                        </button>
                        <form action="{{ route('employees.destroy', $employee->id) }}" method="POST" onsubmit="return confirm('Delete this employee?');">
                            ...
                            <button type="submit" class="flex w-full items-center px-4 py-2 text-sm text-red-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                <span class="material-symbols-outlined text-lg mr-2">delete</span> Delete
                            </button>
                        </form>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-xs text-gray-500 uppercase font-semibold">Base Salary</p>
                        <p class="font-bold text-gray-900 dark:text-white">{{ number_format($employee->base_salary, 0, ',', '.') }} đ</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 uppercase font-semibold">Total Advanced</p>
    ...
    <div x-show="isFormOpen" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen p-4 text-center">
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:max-w-lg w-full p-6">
                <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white mb-4" x-text="editMode ? 'Edit Employee' : 'New Employee'"></h3>
                ...
                <form :action="formAction" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Base Salary (VNĐ)</label>
                    ...
                    <div class="mt-5 sm:flex sm:flex-row-reverse">
                        <button type="submit" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-primary text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary sm:ml-3 sm:w-auto sm:text-sm">Save</button>
                        <button type="button" @click="isFormOpen = false" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-600">Cancel</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="flex items-center justify-center min-h-screen p-4 text-center">
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:max-w-2xl w-full p-6">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white">Advances for <span x-text="currentEmployeeName" class="text-primary"></span></h3>
                    <button @click="isAdvanceOpen = false" class="text-gray-400 hover:text-gray-500"><span class="material-symbols-outlined">close</span></button>
                </div>
                ...
                <form :action="advanceFormAction" method="POST" class="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg mb-6 border border-gray-200 dark:border-gray-600">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
                        ...
                        <div>
                            ...
                            <input type="number" name="amount" required min="1" placeholder="Amount" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white">
                        </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <template x-if="advances.length === 0">
                                <tr>
                                    <td colspan="3" class="px-4 py-4 text-center text-sm text-gray-500">No advance history.</td>
                                </tr>
                            </template>
                        </tbody>
                    </table>
