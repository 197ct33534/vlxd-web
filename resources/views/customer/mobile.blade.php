...
<head>
    ...
    <title>{{ __('Danh sách khách hàng') }} - Mobile</title>
...
<body class="bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-slate-100 antialiased" 
    ...
    >
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-2xl flex flex-col items-center gap-4">
            ...
            <span class="text-slate-700 dark:text-slate-200 font-medium text-sm">Processing...</span>
        </div>
    </div>
    ...
    <header class="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 shadow-sm">
        <div class="flex items-center gap-3">
            <button class="flex h-10 w-10 items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
        ...
        <div class="flex items-center gap-2">
            <button class="relative flex h-10 w-10 items-center justify-center rounded-lg text-slate-600 dark:text-slate-400">
    ...
    <main class="mx-auto max-w-lg p-4 pb-28">
        <div class="mb-6">
            ...
            <form action="{{ route('customers.index') }}" method="GET" class="relative" @submit="isLoading = true">
                ...
                <input name="search" value="{{ request('search') }}" class="w-full rounded-xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 pl-10 pr-4 py-3 text-sm focus:border-primary focus:ring-primary shadow-sm outline-none" placeholder="Tìm kiếm khách hàng..." type="text"/>
            </form>
        </div>
        
        <div class="flex gap-2 overflow-x-auto pb-4 mb-2 custom-scrollbar">
            <a href="{{ route('customers.index', ['status' => 'all']) }}" class="whitespace-nowrap rounded-full {{ request('status', 'all') == 'all' ? 'bg-primary text-white' : 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400' }} px-4 py-1.5 text-xs font-semibold shadow-sm">Tất cả</a>
            <a href="{{ route('customers.index', ['status' => 'active']) }}" class="whitespace-nowrap rounded-full {{ request('status') == 'active' ? 'bg-primary text-white' : 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400' }} px-4 py-1.5 text-xs font-semibold">Đang hoạt động</a>
            <a href="{{ route('customers.index', ['status' => 'inactive']) }}" class="whitespace-nowrap rounded-full {{ request('status') == 'inactive' ? 'bg-primary text-white' : 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400' }} px-4 py-1.5 text-xs font-semibold">Ngừng hoạt động</a>
        </div>
        ...
        <div class="space-y-4">
            ...
            @empty
                <div class="text-center py-10">
                    <p class="text-slate-500">Không tìm thấy khách hàng nào.</p>
                </div>
            @endforelse
        </div>
