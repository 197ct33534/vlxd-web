...
@section('title', __('daily_reports.page_title'))
...
<div class="flex flex-col gap-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-text-light dark:text-text-dark text-3xl font-black tracking-tight">{{ __('daily_reports.title') }}</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('daily_reports.subtitle') }}</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 dark:text-gray-300 uppercase bg-background-light dark:bg-background-dark">
                    <tr>
                        <th class="px-6 py-4 font-bold">{{ __('daily_reports.table.date') }}</th>
                        <th class="px-6 py-4 font-bold">{{ __('daily_reports.table.reporter') }}</th>
                        <th class="px-6 py-4 font-bold">{{ __('daily_reports.table.items') }}</th>
                        <th class="px-6 py-4 font-bold text-center">{{ __('daily_reports.table.status') }}</th>
                        <th class="px-6 py-4 font-bold text-right">{{ __('daily_reports.table.actions') }}</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-primary/5 dark:hover:bg-primary/10 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <span class="bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-300 text-xs font-medium px-2.5 py-0.5 rounded border border-gray-200 dark:border-gray-700">
                                    {{ __('daily_reports.items_count', ['count' => $report->items->count()]) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                @if($report->status === 'approved')
                                    <span class="inline-flex items-center gap-1 bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300">
                                        <span class="material-symbols-outlined text-sm">check_circle</span> {{ __('daily_reports.status.approved') }}
                                    </span>
                                @elseif($report->status === 'rejected')
                                    <span class="inline-flex items-center gap-1 bg-red-100 text-red-800 text-xs font-medium px-2.5 py-0.5 rounded dark:bg-red-900 dark:text-red-300">
                                        <span class="material-symbols-outlined text-sm">cancel</span> {{ __('daily_reports.status.rejected') }}
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 bg-yellow-100 text-yellow-800 text-xs font-medium px-2.5 py-0.5 rounded dark:bg-yellow-900 dark:text-yellow-300">
                                        <span class="material-symbols-outlined text-sm">pending</span> {{ __('daily_reports.status.pending') }}
                                    </span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('daily-reports.show', $report->id) }}" class="text-blue-600 hover:text-blue-900 dark:hover:text-blue-400 font-medium hover:underline">{{ __('daily_reports.view_details') }}</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
